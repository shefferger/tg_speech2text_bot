...
class App:
    ...
    def getlocale(self, meta_text: str, lang: str) -> str:
        return await self.db.get_locale(text_meta=meta_text, lang=lang)

    def register_hanlders(self):
        @self.bot.message_handler(commands=['help', 'start', 'status'])
        async def start_menu_handler(msg):
            cid = msg.chat.id
            status = await self.db.get_status(cid)
            is_enabled = status['is_enabled']
            if not status['lang']:
                lang = 'ru' if msg.from_user.language_code == 'ru' else 'en'
                await self.db.set_status(chat_id=cid, is_enabled=is_enabled, lang=lang)
            else:
                lang = status['lang']
            reply = await self.db.get_locale('bot_status', lang) + \
                await self.db.get_locale('enabled' if is_enabled else 'disabled', lang)
        ...
        async def callback_handler(call):
            ...
            if command == 'on':
                ...
                await self.db.set_status(chat_id=cid, is_enabled=True, lang=lang)
            elif command == 'off':
                kb.add(types.InlineKeyboardButton(str(
                ...
                await self.db.set_status(chat_id=cid, is_enabled=False, lang=lang)
